...
import { existingUser, comparePassword, validatorRequest, bcryptPassword, validatePermissions, verifyUserById } from './utils/validations';
...
export const logInUser=async (req : Request, res : Response  )=>{
    ...
    const validationPassword = comparePassword( password,usuariosAtributte.Password );
...
export const updateStateUsers = async (req : Request, res : Response  )=>{
    ...
    const premissions : Boolean = await validatePermissions(token, Roles.validador);
    if(!premissions){
      return res.status(401).json({
          status:'ERROR',
...
export const updateRolUser = async (req : Request, res : Response  )=>{
    ...
    const premissions : Boolean = await validatePermissions(token, Roles.admin);
    if(!premissions){
